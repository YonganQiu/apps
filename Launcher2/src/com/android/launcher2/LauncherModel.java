...
public class LauncherModel extends BroadcastReceiver {
                    ...
                    try {
                        ...
                        case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
                            ...
                            if (itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION) {
                                ...
                                //Added by yongan.qiu on 2012.3.31 begin.
                                if (intent != null && intent.getComponent() != null && 
                                        "com.android.settings".equals(intent.getComponent().getPackageName())) {
                                    info = getShortcutInfo(manager, intent, c, context, iconTypeIndex,
                                            iconPackageIndex, iconResourceIndex, iconIndex,
                                            titleIndex);
                                } else
                                //Added by yongan.qiu on 2012.3.31 end.
    ...
    /**
     * Make an ShortcutInfo object for a shortcut that isn't an application.
     * <br>Notice that the label of this shortcut is loaded from intent, 
     * not read from datebase, so the label is up to locale.
     */
    private ShortcutInfo getShortcutInfo(PackageManager manager, Intent intent, Cursor c, Context context,
            int iconTypeIndex, int iconPackageIndex, int iconResourceIndex, int iconIndex,
            int titleIndex) {
        ...
        final ResolveInfo resolveInfo = manager.resolveActivity(intent, 0);
        if (resolveInfo != null) {
            info.title = resolveInfo.loadLabel(manager);
        }
        if (info.title == null) {
            info.title = c.getString(titleIndex);
        }
