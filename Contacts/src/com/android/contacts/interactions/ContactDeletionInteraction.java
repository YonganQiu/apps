...
public class ContactDeletionInteraction extends Fragment
    ...
    public static final String ARG_CONTACT_URIS = "contactUris";
    ...
    public void onStart() {
        if (mActive) {
            ...
            args.putParcelable(ARG_CONTACT_URI, mContactUri);
            //begin: added by yunzhou.song
            args.putParcelableArray(ARG_CONTACT_URIS, mContactUris);
            //end: added by yunzhou.song
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
    	//begin: added by yunzhou.song
    	Uri[] contactUris = (Uri[])args.getParcelableArray(ARG_CONTACT_URIS);
    	if(contactUris != null) {
	        return new CursorLoader(mContext,
	                Uri.withAppendedPath(contactUris[0], Entity.CONTENT_DIRECTORY), ENTITY_PROJECTION,
	                null, null, null);
    	}
    	//end: added by yunzhou.song
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        //begin: added by yunzhou.song
        if(mContactUris != null) {
        	mMessageId = R.string.multipleDeleteContactConfirmation;
        	showDialog(mMessageId, mContactUris);
    ...
    //begin: added by yunzhou.song
    public static ContactDeletionInteraction start(
            Activity activity, Uri[] contactUris, boolean finishActivityWhenDone) {
        return startWithTestLoaderManager(activity, contactUris, finishActivityWhenDone, null);
    }
    ...
    static ContactDeletionInteraction startWithTestLoaderManager(
        ...
        if (fragment == null) {
            fragment = new ContactDeletionInteraction();
            fragment.setTestLoaderManager(testLoaderManager);
            fragment.setContactUri(contactUris);
            fragment.setFinishActivityWhenDone(finishActivityWhenDone);
            fragmentManager.beginTransaction().add(fragment, FRAGMENT_TAG)
                    .commitAllowingStateLoss();
        } else {
    ...
    public void setContactUri(Uri[] contactUris) {
        mContactUris = contactUris;
        mActive = true;
        if (isStarted()) {
            Bundle args = new Bundle();
            args.putParcelableArray(ARG_CONTACT_URIS, mContactUris);
    ...
    private void showDialog(int messageId, final Uri[] contactUris) {
        mDialog = new AlertDialog.Builder(getActivity())
                .setTitle(R.string.deleteConfirmation_title)
                .setIconAttribute(android.R.attr.alertDialogIcon)
                .setMessage(messageId)
    ...
    protected void doDeleteContact(Uri[] contactUris) {
        mContext.startService(ContactSaveService.createDeleteContactIntent(mContext, contactUris));
