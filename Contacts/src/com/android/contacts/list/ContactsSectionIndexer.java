...
public class ContactsSectionIndexer implements SectionIndexer {
    ...
    public ContactsSectionIndexer(String[] sections, int[] counts) {
		...
		//begin: added by yunzhou.song at 2012/04/20 for section indexer
		String section;
		ArrayList<String> sectionList = new ArrayList<String>(sections.length);
		HashMap<String, Integer> countMap = new HashMap<String, Integer>(counts.length);
		for (int i = 0; i < sections.length; i++) {
			...
			if (sectionList.contains(section)) {
				countMap.put(section, countMap.get(section).intValue() + counts[i]);
			} else {
				sectionList.add(section);
				countMap.put(section, counts[i]);
			}
		}
		...
		for (int i = 0; i < counts.length; i++) {
			counts[i] = countMap.get(sectionList.get(i));
		}

		sectionList.clear();
		countMap.clear();
		//end: added by yunzhou.song at 2012/04/20 for section indexer
