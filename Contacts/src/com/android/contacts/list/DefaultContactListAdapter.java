...
public class DefaultContactListAdapter extends ContactListAdapter {
    ...
    //begin: added by yunzhou.song
    private final ContactPickerFragment mFragment;
    
    private boolean mFilterPhones;
    private Parcelable[] mExcludeUris;
    
    public DefaultContactListAdapter(Context context, ContactPickerFragment fragment) {
        super(context);
        mFragment = fragment;
    }
    ...
    public void configureLoader(CursorLoader loader, long directoryId) {
        ...
        if(mExcludeUris != null && mExcludeUris.length > 0) {
        	...
        	if(needAppend) {
	        	...
	        	selection.append(Contacts._ID + " NOT IN (");
	        	for(int i = 0; i < mExcludeUris.length; i++) {
	        		if(mExcludeUris[i] != null) {
		...
		if (mFilterPhones
				&& (filter == null || filter.filterType != ContactListFilter.FILTER_TYPE_WITH_PHONE_NUMBERS_ONLY)) {
			if (selection.length() != 0) {
				selection.append(" AND ");
			}
