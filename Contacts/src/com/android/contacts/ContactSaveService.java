...
public class ContactSaveService extends IntentService {
    ...
    //begin: added by yunzhou.song
    public static Intent createDeleteContactIntent(Context context, Uri[] contactUris) {
        Intent serviceIntent = new Intent(context, ContactSaveService.class);
        serviceIntent.setAction(ContactSaveService.ACTION_DELETE_CONTACT);
        serviceIntent.putExtra(ContactSaveService.EXTRA_CONTACT_URIS, contactUris);
        return serviceIntent;
    ...
    private void deleteContact(Intent intent) {
    	//begin: added by yunzhou.song
    	Parcelable[] uris = intent.getParcelableArrayExtra(EXTRA_CONTACT_URIS);
    	if(uris != null) {
    		try {
	    		ContentProviderOperation.Builder builder;
	    		ArrayList<ContentProviderOperation> operations = new ArrayList<ContentProviderOperation>();
	    		for(int i = 0; i < uris.length; i++) {
	    			builder = ContentProviderOperation.newDelete((Uri)uris[i]);
	    			operations.add(builder.build());
	    			if(i != 0 && i % 99 == 0) {
	    				getContentResolver().applyBatch(Contacts.CONTENT_URI.getAuthority(), operations);
	    				operations.clear();
	    			}
	    		}
