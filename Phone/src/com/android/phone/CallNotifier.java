...
public class CallNotifier extends Handler
    ...
    //begin: added by yunzhou.song for preventing sleep when the phone is off hook at 2012/05/22
    private PowerManager.WakeLock mPartialWakeLock;
    //end: added by yunzhou.song for preventing sleep when the phone is off hook at 2012/05/22
    ...
    private CallNotifier(PhoneApp app, Phone phone, Ringer ringer,
        ...
        //begin: remarked by yunzhou.song for preventing sleep when the phone is off hook at 2012/05/22
        ...
        //end: remarked by yunzhou.song for preventing sleep when the phone is off hook at 2012/05/22
    }
    ...
    private void onPhoneStateChanged(AsyncResult r) {
        ...
        if(state != Phone.State.IDLE) {
        	if(!mPartialWakeLock.isHeld()) {
            	if (DBG) log("mPartialWakeLock: acquiring...");
            	mPartialWakeLock.acquire();
            } else {
            	if (VDBG) log("mPartialWakeLock: lock already held.");
            }
        } else {
        	if(mPartialWakeLock.isHeld()) {
            	if (DBG) log("mPartialWakeLock: releasing...");
            	mPartialWakeLock.release();
            } else {
            	if (VDBG) log("mPartialWakeLock: lock already released.");
            }
        }
        //end: added by yunzhou.song for preventing sleep when the phone is off hook at 2012/05/22
